...
public interface BugStateService {
     ...
     * 保存一条 BugState 数据。
     *
     * @param bugState 待保存的数据。
     * @throws BizException 保存失败异常。
     */
    void saveBugState(BugState bugState) throws BizException;

    /**
     * 保存多条 BugState 数据。
     *
     * @param bugStateList 待保存的数据列表。
     * @throws BizException 保存失败异常。
     */
    void saveBugStateList(List<BugState> bugStateList) throws BizException;

    /**
     * 根据id更新 BugState，字段为null的选项会把数据库字段更新为null，即全部更新。
     *
     * @param bugState 更新的目标数据。
     */
    void updateBugState(BugState bugState);
     ...
     */
    void updateBugStateSelective(Map<String, Object> dataMap, Map<String, Object> conditionMap);
     ...
     */
    void logicDeleteBugState(Long id, Long userId) throws BizException;
     ...
     */
    void deleteBugState(Long id) throws BizException;

    /**
     * 根据id查询一条 BugState。
     *
     * @param id 数据唯一id。
     * @return 查询到的 BugState 数据。
     */
    BugStateDTO findBugStateById(Long id);

    /**
     * 根据条件查询得到第一条 BugState。
     *
     * @param params 查询条件
     * @return 符合条件的一个 BugState。
     */
    BugStateDTO findOneBugState(Map<String, Object> params);
     ...
     * @return 查询结果的数据集合。
     */
    List<BugStateDTO> find(Map<String, Object> params, Vector<SortingContext> scs, PagingContext pc);
