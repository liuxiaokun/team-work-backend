...
@Slf4j
@Api("服务表")
public class PermissionServiceController extends BaseController<PermissionService> {

    private final PermissionServicesService permissionServicesService;

    @Autowired
    public PermissionServiceController(PermissionServicesService permissionServicesService) {
    ...
    public Object list(HttpServletRequest request, PermissionServiceDTO permissionServicesDTO) {
        log.info("list:{}", permissionServicesDTO);

        Map<String, Object> params = getConditionsMap(request);
        List<PermissionServiceDTO> list = new ArrayList<>();
    ...
    public Object view(@PathVariable("id") Long id) {
        ...
        PermissionServiceDTO permissionServicesDTO = permissionServicesService.findPermissionServicesById(id);
        return RO.success(permissionServicesDTO);
    }

    @ApiOperation(value = "新增PermissionServices", notes = "新增一条PermissionServices记录")
    @PostMapping(name = "创建")
    public Object create(@RequestBody PermissionServiceDTO permissionServicesDTO, HttpServletRequest request) {
        log.info("add permissionServices DTO:{}", permissionServicesDTO);
        PermissionService sourcePermissionServices = new PermissionService();
        try {
            PermissionService permissionServices = BeanUtil.copyProperties(permissionServicesDTO, sourcePermissionServices);
    ...
    public Object update(@PathVariable("id") Long id, @RequestBody PermissionServiceDTO permissionServicesDTO, HttpServletRequest request) {
        log.info("put modify id:{}, permissionServices DTO:{}", id, permissionServicesDTO);
        PermissionService permissionServices = new PermissionService();
