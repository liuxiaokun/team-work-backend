...
public class PermissionMenuController extends BaseController<PermissionMenu> {
    ...
    private final PermissionMenuService permissionMenusService;

    @Autowired
    public PermissionMenuController(PermissionMenuService permissionMenusService) {
    ...
    public Object list(HttpServletRequest request, PermissionMenuDTO permissionMenusDTO) {
        log.info("list:{}", permissionMenusDTO);

        Map<String, Object> params = getConditionsMap(request);
        List<PermissionMenuDTO> list = new ArrayList<>();
    ...
    public Object view(@PathVariable("id") Long id) {
        ...
        PermissionMenuDTO permissionMenusDTO = permissionMenusService.findPermissionMenusById(id);
        return RO.success(permissionMenusDTO);
    }

    @ApiOperation(value = "新增PermissionMenus", notes = "新增一条PermissionMenus记录")
    @PostMapping(name = "创建")
    public Object create(@RequestBody PermissionMenuDTO permissionMenusDTO, HttpServletRequest request) {
        log.info("add permissionMenus DTO:{}", permissionMenusDTO);
        PermissionMenu sourcePermissionMenus = new PermissionMenu();
        try {
            PermissionMenu permissionMenus = BeanUtil.copyProperties(permissionMenusDTO, sourcePermissionMenus);
    ...
    public Object update(@PathVariable("id") Long id, @RequestBody PermissionMenuDTO permissionMenusDTO, HttpServletRequest request) {
        log.info("put modify id:{}, permissionMenus DTO:{}", id, permissionMenusDTO);
        PermissionMenu permissionMenus = new PermissionMenu();
