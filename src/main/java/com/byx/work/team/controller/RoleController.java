...
import com.byx.work.team.service.RolesService;
 ...
 */
@RestController
@RequestMapping(value = "/roles", name = "角色表")
@Slf4j
@Api("角色表")
public class RoleController extends BaseController<Role> {
    ...
    @ApiOperation(value = "根据条件查询Roles列表", notes = "包含查询条件，分页以及排序功能")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "s", value = "每页的条数", paramType = "query"),
            @ApiImplicitParam(name = "p", value = "请求的页码", paramType = "query"),
            @ApiImplicitParam(name = "sc", value = "排序字段，格式：scs=name(asc),sc=age(desc),有序", paramType = "query"),
            })
    @GetMapping(name = "team-角色表管理")
    public Object list(HttpServletRequest request, RoleDTO rolesDTO) {
        log.info("list:{}", rolesDTO);

        Map<String, Object> params = getConditionsMap(request);
        List<RoleDTO> list = new ArrayList<>();
        int total = rolesService.count(params);
        PagingContext pc = getPagingContext(request, total);
        Vector<SortingContext> scs = getSortingContext(request);
        if (total > 0) {
            list = rolesService.find(params, scs, pc);
        }
        return RO.success(list, pc, scs);
    }

    @ApiOperation(value = "查询Roles", notes = "根据ID查询Roles")
    ...
    public Object view(@PathVariable("id") Long id) {
        log.info("get roles Id:{}", id);
        RoleDTO rolesDTO = rolesService.findRolesById(id);
        return RO.success(rolesDTO);
    }

    @ApiOperation(value = "新增Roles", notes = "新增一条Roles记录")
    @PostMapping(name = "创建")
    public Object create(@RequestBody RoleDTO rolesDTO, HttpServletRequest request) {
        log.info("add roles DTO:{}", rolesDTO);
        Role sourceRoles = new Role();
        try {
            Role roles = BeanUtil.copyProperties(rolesDTO, sourceRoles);
            rolesService.saveRoles(this.packAddBaseProps(roles, request));
        } catch (BizException e) {
            log.error("add roles failed,  rolesDTO: {}, error message:{}", rolesDTO, e.getMessage());
            return RO.error(e.getMessage());
        }
        return RO.success();
    }

    @ApiOperation(value = "修改Roles", notes = "根据ID, 修改一条Roles记录")
    @ApiImplicitParam(name = "id", value = "主键id", dataType = "Long", paramType = "path", required = true, example = "947153698855845888")
    @PutMapping(value = "/{id}", name = "修改")
    public Object update(@PathVariable("id") Long id, @RequestBody RoleDTO rolesDTO, HttpServletRequest request) {
        log.info("put modify id:{}, roles DTO:{}", id, rolesDTO);
        Role roles = new Role();
        roles.setId(id);
        rolesService.updateRoles(this.packModifyBaseProps(BeanUtil.copyProperties(rolesDTO, roles), request));
        return RO.success();
    }

    @ApiOperation(value = "修改Roles", notes = "根据ID, 部分修改一条Roles记录")
    @ApiImplicitParam(name = "id", value = "主键ID", dataType = "Long", paramType = "path", required = true, example = "947153698855845888")
    ...
    public Object updatex(@PathVariable("id") Long id, HttpServletRequest request, @RequestBody Map<String, Object> params) {
        log.info("Patch modify Roles Id:{}", id);
        ...
        rolesService.updateRolesSelective(params, conditions);
        return RO.success();
    }

    @ApiOperation(value = "删除Roles", notes = "根据ID, 逻辑删除一条Roles记录")
    @ApiImplicitParam(name = "id", value = "主键id", dataType = "Long", paramType = "path", required = true, example = "947153698855845888")
    @DeleteMapping(value = "/{id}", name = "删除")
    public Object remove(HttpServletRequest request, @PathVariable("id") Long id) {
        log.info("delete roles, id:{}", id);

        try {
            rolesService.logicDeleteRoles(id, this.getUserId(request));
        } catch (BizException e) {
            log.error("delete failed, roles id: {}, error message:{}", id, e.getMessage());
            return RO.error(e.getMessage());
        }
        return RO.success();
    }
    ...
    public Object sum(@PathVariable String sumField, HttpServletRequest request) {
        ...
        Map<String, Object> params = getConditionsMap(request);
        Double sum = rolesService.sum(sumField, params);
    ...
    public Object groupSum(@PathVariable String groupField, @PathVariable String sumField, HttpServletRequest request) {
        ...
        Map<String, Object> params = getConditionsMap(request);
        Map<String, Double> result = rolesService.groupSum(groupField, sumField, params);
    ...
    public Object groupCount(@PathVariable String groupField, HttpServletRequest request) {
        ...
        Map<String, Object> params = getConditionsMap(request);
        Map<String, Integer> result = rolesService.groupCount(groupField, params);
        return RO.success(result);
    }
}
